...
function Laundromat(){
  ...
  this.wash = function(req, res, next){
    ...
    function iter(index){
      ...
      this._washingMachines[index].call(this, req, this._statusCode, this._url, function(err, mod){
        ...
        if(mod !== undefined){
          ...
          // _.each(['status', 'url'], function(propName){
          //   if(mod[propName] && mod[propName] !== this.['_' + propName]) {
          //     this['_' + propName] = mod[propName];
          //     index = 0; // loop
          //   }
          // });

          if(mod.statusCode && mod.statusCode !== this._statusCode) {
            this._statusCode = mod.statusCode;
            index = 0; // loop
          }

          if(mod.url && mod.url !== this._url) {
            this._url = mod.url;
            index = 0; // loop
          }
