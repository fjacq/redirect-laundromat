...
function stub(obj, propertyName, fn){

  obj['__' + propertyName] = obj[propertyName];
  obj[propertyName] = fn;

  obj[propertyName].restore = function(){
    obj[propertyName] = obj['__' + propertyName];
    delete obj[propertyName].restore;
  };
...
function customRedirect(req, index, pursue){

  return function(statusCode, url){
...
function Laundromat(){
  ...
  this.wash = function(req, res, next){
    ...
    function iter(index){
      ...
      this._res = stub(res, 'redirect', customRedirect(req, index, pursue).bind(this));
